...
RED = (255, 0, 0)
GREEN = (0, 255, 0)
current_color = RED  # Set the initial color to red

# Set thresholds for detecting the touch cycle
downward_peak_threshold = 205  # Value when the sensor is pressed
upward_peak_threshold = 400  # Value when the sensor is released
...
# Initialize sense_value to a safe default
#sense_value = upward_peak_threshold - 100 # Set it to a value above the threshold to start in the non-touch state
...
while running:
    ...
    if ser.in_waiting > 0:
        ...
        if data_decoded:
            ...
            # Check for the upward peak (touch detected)
            if not in_touch_cycle and sense_value > upward_peak_threshold:
                current_color = GREEN  # Turn the box green when touched
                in_touch_cycle = True  # Mark the start of a touch cycle

            # Check for the downward peak (release detected)
            if in_touch_cycle and sense_value < downward_peak_threshold:
